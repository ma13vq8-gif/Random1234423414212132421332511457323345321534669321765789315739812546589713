...
local FLIGHT_KEY = Enum.KeyCode.F
...
UserInputService.InputBegan:Connect(function(input, processed)
	...
	if input.KeyCode == LOCK_KEY then toggleLock() end
	if input.KeyCode == FLIGHT_KEY then flightEnabled = not flightEnabled end
end)
...
local function createSlider(labelText, yPos, initial)
	...
	local function getValue() return math.floor(handle.Position.X.Scale*255) end
	return getValue
end
...
local function ensureHighlight(character)
	...
	if not highlight then highlightCharacter(character) highlight = character:FindFirstChildOfClass("Highlight") end
	return highlight
end
...
RunService.RenderStepped:Connect(function(delta)
	if flightEnabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
		...
		if moveVec.Magnitude>0 then moveVec = moveVec.Unit*flightSpeed end
		flightVelocity.Velocity = moveVec
		if not flightVelocity.Parent then flightVelocity.Parent=HRP end
	else
		flightVelocity.Parent = nil
	end
end)
